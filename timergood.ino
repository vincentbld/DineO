...
void updatePhase() {
  ...
  int startPixel = (currentPhase * totalPixels + PIXEL_OFFSET) % NUMPIXELS;  // Appliquer le décalage et gérer les dépassements
  int endPixel = startPixel + totalPixels;  // Définir le pixel de fin pour le segment
  
  // Colorer tous les pixels du segment avec la couleur de fond
  setColorSegment(startPixel, min(endPixel, NUMPIXELS), backgroundColor);
  
  // Colorer la progression avec la couleur correspondante
  setColorSegment(startPixel, min(startPixel + litPixels, NUMPIXELS), color);
}
