...
// Démarrage automatique après 1 minute sans connexion
unsigned long startupTime;  // Temps au démarrage du serveur
bool hasConnected = false;  // Pour vérifier si une connexion a été établie
...
void setColorSegment(int startPixel, int endPixel, uint32_t color) {
...
// Pulsation d'une couleur donnée
void pulseEffect(uint32_t color, int brightness) {
  static int fadeAmount = 5;
  static int currentBrightness = 0;

  currentBrightness += fadeAmount;
  if (currentBrightness <= 0 || currentBrightness >= brightness) {
...
void setup() {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    hasConnected = true;  // Marquer une connexion
  });
...
void loop() {
  ...
  // Vérifier s'il n'y a pas eu de connexion au bout d'une minute
  if (!hasConnected && (millis() - startupTime > 60000)) {
    Serial.println("Aucune connexion détectée. Démarrage automatique du cycle.");
    startPhase(ENTREE);
    hasConnected = true;  // Marquer comme connecté pour éviter de redémarrer plusieurs fois
  }
